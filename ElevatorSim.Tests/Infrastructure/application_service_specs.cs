...
namespace ElevatorSim.Tests
{
    ...
    public abstract class application_service_spec : IListSpecifications, IDisposable
    {
        ...
        protected void WhenMessage(ICommand command, string testName)
        {
            _when = command;
            _testName = testName;
        }

        public application_service_spec()
        {
            SetUpSpecification();
        }
        ...
        {
            if (!_thenWasCalled)
                Assert.True(false, "THEN was not called from the unit test");
        }

        protected void PrintSpecification()
        {
            Console.WriteLine("Fixture:       {0}", GetType().Name.Replace("_", " "));
            Console.WriteLine("Specification: {0}", _testName.Replace("_", " "));
        ...
        public IEnumerable<SpecificationInfo> ListSpecifications()
        {
            ...
            var myMethods = GetType().GetMethods().Where(m => m.IsDefined(typeof(FactAttribute), true)).ToArray();
